/*
Given an Array find First element that is atleast x
in the given range queries.
...
struct segment_tree{
    ...
    int find_left_most_pos(int val,int x,int lx,int rx){
        if(max_values[x]<val){
            return -1;
        }
        if(rx-lx==1){
        ...
        int m = (lx+rx)/2;
        int left_max_index = find_left_most_pos(val,left(x),lx,m);
        if(left_max_index==-1){
            return find_left_most_pos(val,right(x),m,rx);
        }
        return left_max_index;
    }
    int find_left_most(int val){
        return find_left_most_pos(val,0,0,size);
    }
};
int main(){
    speed
