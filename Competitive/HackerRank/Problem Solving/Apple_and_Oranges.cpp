...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    int apple = 0;
    int orange = 0;
    int n_apples = apples.size();
    int n_oranges = oranges.size();
    vector<int> apple_locations(n_apples);
    vector<int> oranges_loacations(n_oranges);
    for(int i=0;i<n_apples;i++){
        apple_locations[i] = apples[i]+a;
        if(apple_locations[i]>=s&&apple_locations[i]<=t){
            apple++;
        }
    }
    for(int i=0;i<n_oranges;i++){
        oranges_loacations[i] = oranges[i]+b;
        if(oranges_loacations[i]>=s&&oranges_loacations[i]<=t){
            orange++;
        }
    }
