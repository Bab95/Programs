...
*/
void correct(struct Node* root,struct Node* &prev,struct Node* &first,struct Node* &second){
    if(root==NULL){
        return;
    }
    correct(root->left,prev,first,second);
    if(prev&&first==NULL&&prev->data>root->data){
        first = prev;
    }
    if(prev&&first!=NULL&&prev->data>root->data){
        second = root;
    ...
    prev = root;
    correct(root->right,prev,first,second);
}
struct Node *correctBST( struct Node* root )
{
    // add code here.
    ...
    correct(root,prev,first,second);
    if(first!=NULL){
        int tmp = first->data;
